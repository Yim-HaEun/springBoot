...
public class ProductController {
	...
	public String pageList(Model model, @RequestParam(value="page",defaultValue="0") int page) {
		...
		//value="page" 값으로 page이름을 받기로 지정
		//만약에 초반이나 후반에 어떤 값이 page안에 없다면 (page가 null값이라면)
		//기본 값으로 0으로 설정해서 초기값을 null이 아닌 0으로 처리하겠다.
		//페이지는 배열값으로 0이지만 변수에는 추후 1이 할당될 예정
		//페이지에는 1로 표기가되지만 코드안에서는 0부터 시작하는 것으로 표기를 해줘야함
	...
	//상품 전체 목록 페이지로 이동하기위한 GetMapping
