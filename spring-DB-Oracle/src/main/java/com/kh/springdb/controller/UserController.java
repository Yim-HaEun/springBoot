...
*/
	...
	public String updateUserForm(@PathVariable int id, Model model){
		//사용자 정보를 가져와서 모델에 추가해줘야하기 때문에 사용자 정보를 가지고오는 모델 추가코드를 작성
		User user = userService.getUserById(id);
		model.addAttribute("user",user);
		return "user-update";
	}
	...
	public String deleteMappingUser(@PathVariable int id, Model model) {
		userService.deleteUser(id);
		return "redirect:/user-info";
	}
}
...
@RequestMapping : 사용자의 http 요청을 특정 메서드와 할 수 있도록 매핑하는(감싸주는) 역할을 함
	...
	value = "" value는 url 사용자가 요청할 때 주고받는 url을 작성해줌
		예를 들어 value="/user-delete/{id}" 값이 있을 때
			{id}는 PathVariable(패스(경로)변수)로 실제 요청할 경우 해당 url 위치로 들어오는 값을 변수로 활용할 수 있음
	...
	method = "" 메서드에서 처리할 http 요청을 메서드에 지정
		예를 들어 RequestMethod.GET
				...
				RequestMethod.GET
					HTTP GET메서드는 주로 DB에 정보를 요청하기 위해 사용
